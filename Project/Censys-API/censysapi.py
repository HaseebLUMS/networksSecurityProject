...
IPV4_FIELDS = ['ip', '21.ftp.banner.banner', '23.telnet.banner.banner', '80.http.get.title']
...
for d in data:
    try:
        banners[i] = {'banner': d['21.ftp.banner.banner'], 'protocol': 'FTP', 'ip': d['ip']}
        i += 1
    except:
        print('no ftp')
        pass
    try:
        banners[i] = {'banner': d['23.telnet.banner.banner'], 'protocol': 'TELNET', 'ip': d['ip']}
        i += 1
    except:
        print('no telnet')
        pass
    try:
        banners[i] = {'banner': d['80.http.get.title'], 'protocol': 'HTTP', 'ip': d['ip']}
        i += 1
    except:
        print('no http')
        pass
